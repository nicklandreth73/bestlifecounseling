...
export default function Footer() {
  ...
  useEffect( () =>{
    ...
    let footerData = data.find((article) => article.title === footerTitle.title)

    setFooterTitle(footerData)

    let footerTextData = data.find((article) => article.title === footerText.title)
  ...
  return(
    ...
    <EditModal title={edit.title} label={edit.label} article= {edit.article} />
      ...
      <div className="footer-top pt-2">
        <h1 className="footer-text text-center" onClick={() => {
        ...
        }}>{footerTitle.label}</h1>
        <h3 className="footer-text text-center"onClick={() => {
          if(!currentUser || location.pathname.slice(1,5) !== "edit")return
          setEdit({
            open: !edit.open,
        ...
        <h5 className="footer-text text-center"onClick={() => {
          if(!currentUser || location.pathname.slice(1,5) !== "edit")return
          setEdit({
            open: !edit.open,
            title: footerText.title,
